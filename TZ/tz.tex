...
\begin{enumerate}
	...
	\item {for i = 0, i< n, i++ ..... n=}pocet bodu mnoziny p
	\item \hspace{1.5cm} {a = -(p((i+1)\%n).y - p(i\%n).y)\\ 
	\indent \hspace{1.5cm} b = p((i+1)\%n).x - p(i\%n).x }... slozky normaloveho vektoru primky,\\ 
	\indent \hspace{1.5cm} resp. koeficienty a, b obecne rovnice primky ax+by+c=0
	\item \hspace{1.5cm} c = - a * p(i\%n).x - b * p(i\%n).y ... koeficient c obecne rovnice primky
	\item \hspace{1.5cm} if(a*p((i+2)\%n).x + b*p((i+2)\%n).y +c = 0):
	\item \hspace{2.5cm}erase p((i+1)\%n)
